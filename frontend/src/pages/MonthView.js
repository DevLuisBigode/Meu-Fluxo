...
const MonthView = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${API}/stats/month`);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-4xl font-display font-bold tracking-tight">Visão Mensal</h2>
          <p className="text-muted-foreground mt-2">Suas transações deste mês</p>
        </div>
        <Button
          onClick={() => {
            setEditingTransaction(null);
